...
int16_t Max31865_faultToString(uint8_t faultCode, char* faultString)
{
	...
	if (faultCode)
	{
		uart_buf_len = sprintf(faultString, "Fault: \r\n");

		if (faultCode & MAX31865_FAULT_HIGHTHRESH)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "RTD High Threshold\r\n");
		}
		if (faultCode & MAX31865_FAULT_LOWTHRESH)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "RTD Low Threshold\r\n");
		}
		if (faultCode & MAX31865_FAULT_REFINLOW)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "REFIN- > 0.85 x Bias\r\n");
		}
		if (faultCode & MAX31865_FAULT_REFINHIGH)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "REFIN- < 0.85 x Bias - FORCE- open\r\n");
		}
		if (faultCode & MAX31865_FAULT_RTDINLOW)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "RTDIN- < 0.85 x Bias - FORCE- open\r\n");
		}
		if (faultCode & MAX31865_FAULT_OVUV)
		{
			uart_buf_len = sprintf(faultString + uart_buf_len, "Under/Over voltage\r\n");
		}
	}
	else
	{
		uart_buf_len = sprintf(faultString, "No Fault \r\n");
	}
